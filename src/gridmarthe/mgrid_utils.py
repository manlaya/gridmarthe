...
# not very functional, todo better
...
def get_min_layer(ds, aquif_layers=None):
    ...
    # TODO groupby several var with xarray --> avoid DataFrame conversion
...
def plot_outcrop(ds, fout=None, engine=['xr', 'gpd']): #varname='charge', nanval=9999., 
    
    """ plot outcrop layers
    outcrop computation included
    ...
    first_aquif_lay = get_min_layer(ds, aquif_layers=None) # no subset, get all layers / varname=varname, nanval=nanval, 
    maxn = np.max(ds.z.data)
    ...
    if engine == 'xr':
        first_aquif_lay2 = assign_coords(first_aquif_lay, add_lay=False)
        the_plot = first_aquif_lay2['z'].plot.pcolormesh(
            x='x', y='y',
            ax=ax,
            cmap=cmap,
            levels=bounds,
        ...
        first_aquif_lay3 = nc_to_gdf(first_aquif_lay)
        first_aquif_lay3.plot(
            column='z',
            ax=ax,
            cax=cax,
            cmap=cmap,
...
def nc_to_gdf(*args, **kwargs):
    print("deprecated, please use `to_geodataframe()`")
    return to_geodataframe(*args, **kwargs)
...
def plot_nested_grid(da, ax=None, var='charge'):
    ...
    for dx2, dy2 in zip(dx, dy):
        gig = da.where(da['dx'] == dx2, drop=True)
        gig[var].plot.pcolormesh(x='x', y='y', ax=ax, vmin=vmin, vmax=vmax, add_colorbar=False)
    grid[var].plot.pcolormesh(x='x', y='y', ax=ax, vmin=vmin, vmax=vmax)
